...
if (angle > 0)
  ...
  if (angle < -35)
  {
    ...
    if (angle > 0)
      {
        vTorque = angle * 12 + 0.7 * newTorque;
        newTorque = vTorque;
        }
    if (angle < 0)
      {
        vTorque = angle * 12 + 0.7 * newTor;
        newTor = vTorque;
        }

    if (angle < -1)
      {
        vTorque = angle * 4 + 0.7 * newTor;
        newTor = vTorque;
      }
    if (angle > 1)
      {
        vTorque = angle * 4 + 0.7 * newTorque;
        newTorque = vTorque;
      }

    if (angle < -3)
      {
        vTorque = angle * 8 + 0.7 * newTor;
        newTor = vTorque;
      }
    if (angle > 3)
      {
        vTorque = angle * 8 + 0.7 * newTorque;
        newTorque = vTorque;
      }

      if (angle < -5)
      {
        vTorque = angle * 10 + 0.7 * newTor;
        newTor = vTorque;
      }
    if (angle > 5)
      {
        vTorque = angle * 10 + 0.7 * newTorque;
        newTorque = vTorque;
      }
    if (angle < -10)
      {
        vTorque = angle * 14 + 0.7 * newTor;
        newTor = vTorque;
      }
    if (angle > 10)
      {
        vTorque = angle * 14 + 0.7 * newTorque;
        newTorque = vTorque;
      }
